...
Repare que uma nova pasta chamada `static` (o mesmo nome que demos no `/meu_site/consoles/settings.py`) foi criada. Lá dentro, temos uma cópia de todos os arquivos estáticos do nosso projeto e aplicativos. Caso esse comando seja executado uma segunda vez, os arquivos dessa pasta serão atualizados com as alterações feitas nos arquivos estáticos de dentro dos aplicativos (por isso a mensagem avisando).
